...
function _refresh(url, callback) {
  ...
  httpRequest.onreadystatechange = function() {
    if (httpRequest.readyState == 4) {
      // Replace <> with \x3c and \x3e to avoid script injection.
      var responseText = httpRequest.responseText;
      responseText = responseText.replace(/</g, '\\x3c').replace(/>/g, '\\x3e');
      _feed = callback;
      eval('(' + responseText + ')');
      httpRequest = null;
    }
  ...
  // Set encoding to utf-8 to avoid script injection.
  httpRequest.setRequestHeader('Content-Type', 'application/javascript');
