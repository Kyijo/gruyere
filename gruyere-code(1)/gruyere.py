...
class GruyereRequestHandler(BaseHTTPRequestHandler):
    ...
    def _DoDeletesnippet(self, cookie, specials, params):
        ...
        # Get the token
        csrf_token = self._GetParameter(params, 'csrf_token')

        # Verify the token
        if not _VerifyXsrfToken(self, cookie[COOKIE_UID], csrf_token, 'deletesnippet'):
...
def _GenerateXsrfToken(self, cookie, action):
    ...
    timestamp = time.time()
    data = cookie_secret + cookie + str(timestamp) + action
    token = hashlib.sha256(data.encode()).hexdigest()
    return str(timestamp) + "|" + token


def _VerifyXsrfToken(self, cookie, action_token, action):
    ...
    # Regenerate the token with the same timestamp and action, and check that it matches the user supplied value
    data = cookie_secret + cookie + action_time + action
    hash_to_verify = hashlib.sha256(data.encode()).hexdigest()
