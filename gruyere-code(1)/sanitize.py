...
def _SanitizeTag(t):
    ...
    # Check each attribute and make sure its not disallowed
    for a in disallowed_attributes:
        attr_ = t.lower().find(a)
        if attr_ != -1:
            # If the attribute is disallowed, remove it. We find the start and the end of the disallowed attribute and replace it with blocked
            start = t.find('"', attr_)
            end = t.find('"', start + 1)
            if start != -1 and end != -1:
                t = t[:start + 1] + 'blocked' + t[end:]
    return t
